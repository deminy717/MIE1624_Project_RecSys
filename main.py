...
def predict(model, data_loader, opt):
    ...
    with torch.no_grad():
        for idx, (test_data, scores) in enumerate(data_loader):
            ...
            final_prediction = final_prediction + output.tolist()
            mse_loss = torch.sum((output-scores)**2)
            total_loss += mse_loss.item()

            mae_loss = torch.sum(abs(output-scores))
            total_maeloss += mae_loss.item()
    #print('num test', len(final_prediction))
    ...
    model.train()
    #np.save("./dataset/Digital_Music_data/test/test_prediction.npy", final_prediction)
