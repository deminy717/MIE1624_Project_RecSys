...
MAX_VOCAB = 50000
DOC_LEN = 500
PRE_W2V_BIN_PATH = "google.bin"  # the pre-trained word2vec files
...
def build_doc(u_reviews_dict, i_reviews_dict, vocab = None):
    '''
    1. extract the vocab
...
if __name__ == '__main__':
    ...
    data = pd.read_csv(filename)[['reviewerID', 'itemID', 'overall', 'reviewText', 'summary']]
    data["reviewText"] = data["reviewText"] + data["summary"] + data["summary"]
    data.drop(columns=['summary'], inplace=True)
    data.rename(columns={"reviewerID": "user_id", "itemID": "item_id", "overall": "ratings", "reviewText": "reviews"},
                inplace=True)
